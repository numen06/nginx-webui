...
class NginxConfig(BaseModel):
    ...
    config_path: str = "/app/nginx/nginx.conf"
    executable: str = "/usr/sbin/nginx"
    static_dir: str = "/app/backend/static"
    log_dir: str = "/app/data/logs"
    conf_dir: str = "/app/nginx/conf.d"
    access_log: str = "/app/data/logs/access.log"
    ...
    build_logs_dir: str = "/app/data/nginx/build_logs"
...
class ConfigManager:
    ...
    def load_config(self) -> Config:
        ...
        nginx_base_dir = os.getenv("NGINX_DIR", "/app/nginx").rstrip("/")

        nginx_config["config_path"] = os.getenv(
            "NGINX_CONFIG_PATH",
            nginx_config.get("config_path", f"{nginx_base_dir}/nginx.conf"),
        )
        nginx_config["executable"] = os.getenv(
            "NGINX_EXECUTABLE", nginx_config.get("executable", "/usr/sbin/nginx")
        )
        nginx_config["static_dir"] = os.getenv(
            "NGINX_STATIC_DIR", nginx_config.get("static_dir", "/app/backend/static")
        )
        nginx_config["log_dir"] = os.getenv(
            "NGINX_LOG_DIR", nginx_config.get("log_dir", f"{data_root}/logs")
        )
        nginx_config["conf_dir"] = os.getenv(
            "NGINX_CONF_DIR",
            nginx_config.get("conf_dir", f"{nginx_base_dir}/conf.d"),
        )
        nginx_config["access_log"] = os.getenv(
            "NGINX_ACCESS_LOG",
            nginx_config.get("access_log", f"{data_root}/logs/access.log"),
        )
        nginx_config["error_log"] = os.getenv(
            "NGINX_ERROR_LOG",
            nginx_config.get("error_log", f"{data_root}/logs/error.log"),
        )
        nginx_config["ssl_dir"] = os.getenv(
            "NGINX_SSL_DIR", nginx_config.get("ssl_dir", f"{data_root}/ssl")
        )
        ...
        nginx_config["versions_root"] = os.getenv(
            "NGINX_VERSIONS_ROOT",
            nginx_config.get("versions_root", f"{data_root}/nginx/versions"),
        )
        nginx_config["build_root"] = os.getenv(
            "NGINX_BUILD_ROOT",
            nginx_config.get("build_root", f"{data_root}/nginx/build"),
        )
        nginx_config["build_logs_dir"] = os.getenv(
            "NGINX_BUILD_LOGS_DIR",
            nginx_config.get("build_logs_dir", f"{data_root}/nginx/build_logs"),
        )
        ...
        backup_config["max_backups"] = int(
            os.getenv("MAX_BACKUPS", backup_config.get("max_backups", 10))
        )
        backup_config["backup_dir"] = os.getenv(
